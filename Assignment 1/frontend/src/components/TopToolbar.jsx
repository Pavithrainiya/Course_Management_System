...
import { Shield, UserCheck, GraduationCap, LogOut, Search, Sparkles, User, Settings } from 'lucide-react';
...
export const TopToolbar = () => {
  ...
  const getPageTitle = () => {
    ...
    if (path === '/certificates') return 'Certificates & QR Code Verification';
    if (path === '/admin') {
  ...
  return (
    <>
          ...
          {user && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', paddingLeft: '14px', borderLeft: '1px solid var(--border-glass)' }}>
              <div
                ...
                onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--accent-cyan)'; e.currentTarget.style.background = 'rgba(0, 198, 255, 0.1)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--border-glass)'; e.currentTarget.style.background = 'rgba(255, 255, 255, 0.04)'; }}
              >
                <div style={{
                  width: '32px',
                  height: '32px',
      ...
      {/* User Profile Side Navbar Drawer */}
